...
# 直接用json也行
data_dict={'train': 'dataset/coco-yolo/images/train2014', 'val': 'dataset/coco-yolo/images/val2014', 'test': 'dataset/coco-yolo/images/test2014', 'anno_path': 'dataset/coco-yolo/annotations/instances_val2014.json', 'nc': 80, 'is_coco': True, 'names': ['person', 'bicycle', 'car', 'motorcycle', 'airplane', 'bus', 'train', 'truck', 'boat', 'traffic light', 'fire hydrant', 'stop sign', 'parking meter', 'bench', 'bird', 'cat', 'dog', 'horse', 'sheep', 'cow', 'elephant', 'bear', 'zebra', 'giraffe', 'backpack', 'umbrella', 'handbag', 'tie', 'suitcase', 'frisbee', 'skis', 'snowboard', 'sports ball', 'kite', 'baseball bat', 'baseball glove', 'skateboard', 'surfboard', 'tennis racket', 'bottle', 'wine glass', 'cup', 'fork', 'knife', 'spoon', 'bowl', 'banana', 'apple', 'sandwich', 'orange', 'broccoli', 'carrot', 'hot dog', 'pizza', 'donut', 'cake', 'chair', 'couch', 'potted plant', 'bed', 'dining table', 'toilet', 'tv', 'laptop', 'mouse', 'remote', 'keyboard', 'cell phone', 'microwave', 'oven', 'toaster', 'sink', 'refrigerator', 'book', 'clock', 'vase', 'scissors', 'teddy bear', 'hair drier', 'toothbrush']}
...
def bbox_decode(anchor_points, pred_dist):
    reg_max=16
    proj = nn.Parameter(torch.linspace(0, reg_max, reg_max + 1), requires_grad=False)
    batch_size, n_anchors, _ = pred_dist.shape
    pred_dist = F.softmax(pred_dist.view(batch_size, n_anchors, 4, reg_max + 1), dim=-1).matmul( proj.to(pred_dist.device))   
...
anchors, anchor_points, n_anchors_list, stride_tensor = generate_anchors(feats=feats)
# print(anchors.shape)          #torch.Size([8400, 4])
# print(anchor_points.shape)    #torch.Size([8400, 2])
# print(n_anchors_list)         #[6400, 1600, 400]
# print(stride_tensor.shape)    #torch.Size([8400, 1])

# pboxes
anchor_points_s = anchor_points / stride_tensor
pred_bboxes = bbox_decode(anchor_points_s, pred_distri)  # xyxy
# print(anchor_points_s.shape)  #torch.Size([8400, 2])
# print(pred_bboxes.shape)      #torch.Size([2, 8400, 4])

# rescale bbox
target_bboxes /= stride_tensor

# # bbox loss
# loss_iou, loss_dfl = bbox_loss(pred_distri, pred_bboxes, anchor_points_s, target_bboxes, target_scores, target_scores_sum, fg_mask)
